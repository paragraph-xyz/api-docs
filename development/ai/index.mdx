...
The [Paragraph MCP server](/development/mcp) connects AI agents directly to the Paragraph API. Manage posts, search content, work with coins, and more from any MCP-compatible client.

The fastest way to connect is using the hosted server at `mcp.paragraph.com` — no installation or API key management required:
...
You can also [run the server locally](/development/mcp#local-server) with `npx @paragraph-com/mcp` if you prefer. See the [full MCP setup guide](/development/mcp) for all options.
